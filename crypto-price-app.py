...
# To know more about streamlit cache: https://docs.streamlit.io/en/stable/api.html#optimize-performance
...
def load_data():
    ...
    global_metrics = coin_data['props']['initialProps']['pageProps']['globalMetrics']
    ...
    df = pd.DataFrame(columns=['coin_name', 'coin_symbol', 'market_cap',
...
# Changing y-axis label based on number of digits
number_of_digits = len(str(int(selected_coin_df['market_cap'].max())))
...
@st.cache(ttl=86400, max_entries=25)
...
historical_timeseries_df = get_historical_time_series_data(selected_crypto, week_ago, today)
...
# Select columns
selected_coin_price_info_df = selected_coin_df[['coin_name', 'coin_symbol', 'market_cap', 'price', 'volume_24h']]
